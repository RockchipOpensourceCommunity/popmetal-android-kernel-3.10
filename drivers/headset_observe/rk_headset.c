...
//	rk28_send_wakeup_key();
	headset_info->cur_headset_status = BIT_HEADSET_NO_MIC; 
	switch_set_state(&headset_info->sdev, BIT_HEADSET_NO_MIC);//headset_info->cur_headset_status);	
	#if defined(CONFIG_SND_RK_SOC_RK2928) || defined(CONFIG_SND_RK29_SOC_RK610)
...
//	printk("headset_timer_callback,headset->headset_status=%d\n",headset->headset_status);	
	...
	#endif
//	rk28_send_wakeup_key();
	switch_set_state(&headset_info->sdev, BIT_HEADSET_NO_MIC);
	
	printk("headset_info->cur_headset_status = %d\n",headset_info->cur_headset_status);	
