...
static void armv7_save_pmu_regs(struct arm_pmu *cpu_pmu,
...
static void armv7_restore_pmu_regs(struct arm_pmu *cpu_pmu,
					struct cpupmu_regs *regs)
{
	unsigned int cnt;
	if (!(regs->pmc & ARMV7_PMNC_E))
...
static void armv7pmu_enable_event(struct perf_event *event)
{
	...
	if (!armv7_pmnc_counter_valid(cpu_pmu, idx)) {
		pr_err("CPU%u operation on invalid PMNC counter %d\n",
						smp_processor_id(), idx);
		return;
	}
...
static void armv7pmu_init(struct arm_pmu *cpu_pmu)
{
	...
	cpu_pmu->save_regs	= armv7_save_pmu_regs;
	cpu_pmu->restore_regs	= armv7_restore_pmu_regs;
